...
static Real Lambda_fac, Lambda_fac_time;
  ...
  for (int k=kl; k<=ku;++k) { 
	...
	if (temp>T_floor) {
	  ...
	  if (temp<T_cut){
	    ...
	    Real cLfrac = Lambda_fac;

	    Real temp_new = max(cooler->townsend(temp_cgs,rho_cgs,dt_cgs,cLfrac), T_floor);
...
void Source(MeshBlock *pmb, const Real time, const Real dt,
		const AthenaArray<Real> &prim, const AthenaArray<Real> &prim_scalar,
		const AthenaArray<Real> &bcc, const AthenaArray<Real> &cons,
		AthenaArray<Real> &cons_scalar) {
  ...
  if (cooling_flag != 0){
    ...
    townsend_cooling(pmb, time, dt, prim, prim_scalar, bcc, cons, cons_scalar);
  }
  if (shift_flag != 0){
    if (!shift_flag_print_count){
      shift_flag_print_count = true;
    }
  frme_shift(pmb, time, dt, prim, prim_scalar, bcc, cons, cons_scalar);
  }
  return;
}
  ...
  for (int k=ks; k<=ke; k++) {
    ...
    for (int j=js; j<=je; j++) {
      ...
      for (int i=is; i<=ie; i++) {
        ...
        phydro->u(IM3,k,j,i) += phydro->u(IDN,k,j,i) * v_shift0;
