  ...
  for (int k=ks; k<=ke; k++) {
    ...
    for (int j=js; j<=je; j++) {
      ...
      for (int i=is; i<=ie; i++) {
        ...
        phydro->u(IM3,k,j,i) = phydro->u(IDN,k,j,i) * A_KH * sin(4*PI*x1) * exp(-1.0*x3*x3/front_thick/front_thick) * sin(k_x*x1) * sin(k_y*x2);
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          //Internal energy
          phydro->u(IEN,k,j,i) += (temp/(KELVIN*mu))*amb_rho/(g-1);
        }
